...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        cv2.rectangle(img_new, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img_new, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)

    return img_new

def plot_one_box_colab(x, img, color=None, label=None, line_thickness=3):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
